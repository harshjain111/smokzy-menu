...
function row2libraryFlavor(r) {
  ...
  return { id: r.id, name: r.name, blendType: r.blend_type || 'signature',
           strength: r.strength, description: r.description, notes: r.notes || [],
...
// merged: returns each pot with both its inline flavours and its library-linked flavours
...
// ---- INLINE FLAVORS (legacy support) ----
async function createFlavor(potId, d) {
  const id = d.id || randomId();
  const row = { id, pot_id: potId, name: d.name || '', strength: d.strength || 5,
...
async function updateFlavor(potId, flavorId, p) {
  const u = {};
  if (p.name !== undefined) u.name = p.name;
  if (p.strength !== undefined) u.strength = p.strength;
  if (p.description !== undefined) u.description = p.description;
...
// ---- LIBRARY (master flavour catalogue) ----
...
async function linkLibraryToPot(potId, libraryIds) {
  // libraryIds is an array of library_ids to link
...
async function getAnalytics() {
  ...
  const nameById = {};
  if (flavorIds.length) {
    // resolve from inline flavors AND library
...
async function getEverything() {
  ...
  return {
    ...
    pots, pairings, feedback, analytics, library
  };
}
